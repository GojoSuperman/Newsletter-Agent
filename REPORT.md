...
노드는 의존성(`http_get`, `ask`, `extract`, `post`, `rewrite`)을 인자로 받는 순수 함수이고 `graph.py`의 `real_nodes()`가 `functools.partial`로 묶어 등록합니다. 그래서 테스트 90개가 네트워크·OpenAI·Discord 없이 돕니다. `tests/test_e2e.py`는 실노드 다섯 개를 가짜 RSS·LLM·웹훅으로 이어 붙여 수집→발행을 한 번에 돌리며, 죽은 소스 격리·면제·팬아웃·재생성 통과·웹훅 1회 호출까지 한 테스트에서 확인합니다.
...
LLM 호출 자체가 실패하는 경우(재시도 후에도 오류)도 실행을 멈추지 않습니다. 취재 실패는 그 기사를 `취재 실패 → 제외`로, 검수 호출 실패는 `검수 실패 → 스킵`으로 건너뛰며, 검수를 거치지 않은 글은 절대 발행되지 않습니다. 소스가 죽으면 ①이 격리하고, 발행할 게 0건이면 ⑤가 보내지 않습니다. 즉 파이프라인이 예외로 멈추는 곳은 웹훅 실패와 설정 오류(시작 시점)뿐입니다.
